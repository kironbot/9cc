...
Type *type_specifier() {
    ...
    for (;;) {
        ...
        case LONG + INT:
            ...
            break;
        case 0:
            // If there is no type specifier, it becomes int.
            // e.g. `typedef x` means `typedef int x`
...
VarList *read_func_param() {
    ...
    // "array of T" is converted to "pointer to T" only in the prameter
    // context. e.g. *argv[] is converted to **argv by this.
    if (ty->kind == TY_ARRAY)
...
// This function returns true if it looks like we are at the
...
Initializer *gvar_initializer(Initializer *cur, Type *ty) {
    ...
    if (n->kind == ND_ADDR) {
        if (n->lhs->kind != ND_VAR)
            error_tok(tok, "invalid initializer");
        return new_init_label(cur, n->lhs->var->name);
    }
...
// Creates a node for an array access. ex) if var represents
...
//   ex) `char x[4] = "foo"` is equivalent to `char x[4] =
//   {'f', 'o', 'o', '\0'}`.
//
// - If a rhs is an incomplete array, its size is set by
//   counting the number of items on the rhs.
//   ex) `x[] = {1,2,3}` has type `int[3]`
